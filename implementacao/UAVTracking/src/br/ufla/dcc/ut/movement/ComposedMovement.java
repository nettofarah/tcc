...
public class ComposedMovement extends MovementManager {
	 ...
	 */
	private static final Logger LOGGER = Logger.getLogger(ComposedMovement.class.getName());
...
*/
	...
	public final Collection<Movement> createMoves(Collection<Node> allNodes)
	{
		...
		Map<String, Integer> nameAndAmmount = Configuration.getInstance().getNameAndAmmount();
		...
		for (String nType : types) {
			...
			for (String stype : types) {
				...
				if(stype.equals("UAV") ){
					movs.add(new RandomWalkColisionAvoidance());
				}
		...
		{
		   Collection<Movement> nextMovesaux = movs.get(type).createMoves(groupNodes.get(type));
